...
struct
  ...
  let out e =
    match T.repr e with
    | Bvar(k,_) -> (*ignore (Printf.printf "bvar");*) Bind k
    | Fvar x -> (*ignore (Printf.printf "fvar");*) Atom( Plib.to_string Var.pretty x )
    | True -> (*ignore (Printf.printf "true");*) Atom "true"
    | False -> (*ignore (Printf.printf "false");*) Atom "false"
    | Kint z -> (*ignore (Printf.printf "int");*) Atom (Z.to_string z)
    | Kreal r -> (*ignore (Printf.printf "real");*) Atom (Q.to_string r)
    | Times(z,e) when Z.equal z Z.minus_one -> Unop("-",e)
    | Times(z,e) -> Hbox("*",[e_zint z;e])
    | Add es -> (*ignore (Printf.printf "add");*) Sum es
    | Mul es -> (*ignore (Printf.printf "mul");*) Hbox("*",es)
    | Div(a,b) -> (*ignore (Printf.printf "div");*) Binop(a,"div",b)
    | Mod(a,b) -> (*ignore (Printf.printf "mod");*) Binop(a,"mod",b)
    | And es -> (*ignore (Printf.printf "and");*) Vbox("/\\",es)
    | Or  es -> (*ignore (Printf.printf "or");*) Vbox("\\/",es)
    | Not e -> (*ignore (Printf.printf "not");*) Unop("not ",e)
    | Imply(hs,p) -> (*ignore (Printf.printf "imply");*) Vbox("->",hs@[p])
    | Eq(a,b) -> (*ignore (Printf.printf "eq");*)
        if T.sort e = Sprop
        then Vbox("<->",[a;b])
        else Hbox("=",[a;b])
    | Lt(a,b) -> Hbox("<",[a;b])
    | Neq(a,b) -> Hbox("!=",[a;b])
    | Leq(a,b) -> Hbox("<=",[a;b])
    | Fun(a,es) -> (*ignore (Printf.printf "call");*) Call(a,es)
    | Apply(e,es) -> (*ignore (Printf.printf "apply");*) Closure(e,es)
    | If(c,a,b) -> (*ignore (Printf.printf "cond");*) Cond(c,a,b)
    | Acst(_,v) ->(*ignore (Printf.printf "const");*)  Const(v)
    | Aget(a,b) -> (*ignore (Printf.printf "get");*) Access(a,b)
    | Aset(a,b,c) ->(*ignore (Printf.printf "set");*)  Update(a,b,c)
    | Logic.Bind(q,x,e) -> (*ignore (Printf.printf "bind");*) abstraction [q,x] (lc_repr e)
  ...
  let named_out env e = (*e is a term env.named is a map from term to string*)
    try Atom(Tmap.find e env.named)
    with Not_found -> (*ignore (Printf.printf "not atom");*) out e
  ...
  and pp_free_out env fmt = function
    | Bind k -> pp_print_string fmt (find_var env k)
    | Atom x -> (*ignore (Printf.printf "free out");*) pp_print_string fmt x
    | Call(f,es) -> pp_call env fmt f es
      ...
      let env =
        List.fold_left
          (fun env t ->
             ...
             env_x)
          env ts in
      (*ignore (Printf.printf "pp share");*)
      pp_free env fmt t ;
      (*ignore (Printf.printf "pp share2");*)
      fprintf fmt "@]" ;
    end
  ...
  let pp_term_env (env:env) (fmt:formatter) t = (*ignore (Printf.printf "term_env");*) pp_share env fmt t
