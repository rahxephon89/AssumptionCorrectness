...
struct
  ...
  let havoc w xs =
    (*ignore(Printf.printf "havoc called ignore");
    let () = Printf.printf "havoc called %s." C.self in*)
    let ys = H.Set.mapping newchunk xs in
    (* xs: (H : Qed.Collection.S with type t = C.t) *)
    (* (C : Sigs.Chunk)*)
    (*called by cc_lval ?xs:  C.M.domain = C.M.Heap.set*)
    (*ys:  H.Set.mapping = Lang.F.var H.map *)
    (*w.map: Lang.F.var H.map*)
    (*?w: C.L.sigma = C.M.Sigma.t*)
