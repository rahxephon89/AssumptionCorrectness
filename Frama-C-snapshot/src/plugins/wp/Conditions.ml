...
let rec pred_cond_when = function
  | When p -> Some p
  | _ -> None

let rec prec_cond_notwhen pc = match pc with
  | When p -> None
  | _ -> Some (pred_cond pc)


let extract_when bundle = let wl = Bundle.map pred_cond_when bundle in
                    let nwl = Bundle.map prec_cond_notwhen bundle in
                    let wl_ = List.filter (fun p -> p != None) wl in
                    let nwl_ = List.filter (fun p -> p != None) nwl in
                    (wl_,nwl_)
...
let assume_when ?descr ?stmt ?deps ?warn ?(init=false) p hs =
  match F.is_ptrue p with
  | Yes -> hs
  | No ->
      let cond = if init then Init p else When p in
      let s = step ?descr ?stmt ?deps ?warn cond in
      Bundle.add s Bundle.empty
  | Maybe ->
      begin
