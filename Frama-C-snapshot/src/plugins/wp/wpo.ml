...
let reduce g =
  ...
  | GoalAnnot vc ->    (*let () = Printf.printf "reduce" in let () = Wp_parameters.debug ~dkey "reduce:%a"
        F.pp_pred vc.goal.obligation in*) WpContext.on_context (get_context g) VC_Annot.resolve vc
