...
struct
  ...
  let pp_term fmt e =
    (*if Wp_parameters.has_dkey dkey_pretty then*)
    (*then QED.debug fmt e
    else*)
      match Context.get_opt context_pp with
      | Some env -> (*let () = ignore (Printf.printf "pp_term some") in*) Pretty.pp_term_env env  fmt e
      | None -> (*let () = ignore (Printf.printf "pp_term None") in*)
          let env = Pretty.known Pretty.empty (QED.vars e) in
          Pretty.pp_term env fmt e

  let pred_transform  = transform_term

  let add_node = QZERO.add_node

  let add_node_eq = QZERO.add_node_eq

  let update_namespace_term = QZERO.update_namespace_term

  let dag_copy = QZERO.dag_copy
  
  let dag_copy_replace = QZERO.dag_copy_replace

  module NameSpace = QZERO.NameSpace

  let update_term_name = QZERO.update_term_name
  
  let add_name_fun = QZERO.add_name_fun
...
let dkey = Wp_parameters.register_category "pretty" (* Debugging key *)
let debug fmt = Wp_parameters.debug ~dkey fmt


let freshvar ?basename tau = let () = (debug "freshvar = @[<hov2>  %a@]@." (Pretty.pp_tau) (tau)) in F.fresh (Context.get cpool) ?basename tau
