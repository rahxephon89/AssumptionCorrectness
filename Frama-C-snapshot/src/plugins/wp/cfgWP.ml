...
open Wpo
  ...
  let dkeyoutput = Wp_parameters.register_category "VCGen"

  let debug fmt = Wp_parameters.debug ~dkey fmt


  let debug2 fmt = Wp_parameters.debug ~dkey:dkeyoutput fmt
...
let pp_vc fmt vc =
    ...
    Splitter.iter
      (fun tags vc ->
         ...
         begin
           ...
           | [] -> ()
           | t::ts ->
               Format.fprintf fmt "vcs999(%a" Splitter.pretty t ;
               List.iter (fun t -> Format.fprintf fmt "vcs999,%a" Splitter.pretty t) ts ;
               Format.fprintf fmt ")@\n" ;
         end ;
  ...
  (*let obtain_vcs gvcs =
      ...
      ignore (debug "enter assume_vc_vc_999 = @[<hov2>  %a@]@." pp_vc vc) 
      
  end*)

  let rec imply_chain lst = match lst with
                              | [] -> p_true
                              | p_::[] -> p_
                              | l::lst_ -> p_imply l (imply_chain lst_)

  let rec removeOption l l1 = match l with
                          | [] -> l1
                          | Some p::l_ -> (p::removeOption l_ l1)
                          | _::l_-> removeOption l_ l1

(*
  let rec extractBundle_ bLst l = 
      match bLst with
      |[] -> l
      |b::bLst1 -> ignore(debug "extractBundle: @[<hov2>  %a@]@.\n" Pcond.dump b); 
      let (ext_when, ext_others) = Conditions.extract_when b in
      let ext_when =  removeOption ext_when [] in
      let ext_others = removeOption ext_others [] in
      let (l1,l2) = extractBundle_ bLst1 l in
      (ext_when@l1, ext_others@l2)
*)
      (*let extb = Conditions.extract b in ignore (List.iter (debug "condition extract: @[<hov2>  %a@]@.\n" Lang.F.pp_pred) extb); List.append extb  (extractBundle bLst1 l)*)

  let vc_collect_ vc l = let (ext_when, ext_others) = Conditions.extract_when vc.hyps in
                        let ext_when =  removeOption ext_when [] in
                        let ext_others = removeOption ext_others [] in  
                        let other = p_conj ext_others in                        
                        let goal = if ext_when == [] then vc.goal else imply_chain (ext_when@[vc.goal]) in
                        (p_conj [goal;other])::l     


  let vc_collect__ vc l = let (ext_when, ext_others) = Conditions.extract_when vc.hyps in
                        let ext_when =  removeOption ext_when [] in
                        let ext_others = removeOption ext_others [] in  
                        let other = p_conj ext_others in                        
                        let goal = if ext_when == [] then vc.goal else imply_chain (ext_when@[vc.goal]) in
                        (imply_chain [other;goal])::l
                

  let vc_collect vc l = ignore (debug "check vc collect: @[<hov2>  %a@]@." pp_vc vc); ignore (List.iter (debug "check vc list: %a" Pcond.dump) l); vc.hyps::l


  let vc_collect_goal vc l = vc.goal::l
...
*)
  ...
  let vcs_collect_goal vcs = 
    Splitter.fold
      (fun tags vc ->
         vc_collect_goal vc)
    vcs 

  let gvcs_collect_ gvcs =
    Gmap.fold_sorted
      (fun goal vcs ->
         vcs_collect vcs vc_collect_
      ) gvcs 

  let gvcs_collect__ gvcs =
    Gmap.fold_sorted
      (fun goal vcs ->
         vcs_collect vcs vc_collect__
      ) gvcs

  let gvcs_collect gvcs =
    Gmap.fold_sorted
      (fun goal vcs ->
         vcs_collect vcs vc_collect
      ) gvcs 

  let gvcs_collect_goal gvcs  =
    Gmap.fold_sorted
      (fun goal vcs ->
         vcs_collect_goal vcs 
      ) gvcs 
 
  

 let rec extractBundle bLst l = 
      match bLst with
      |[] -> l
      |b::bLst1 -> let extb = Conditions.extract b in ignore (List.iter (debug "condition extract: @[<hov2>  %a@]@.\n" Lang.F.pp_pred) extb); List.append extb  (extractBundle bLst1 l)
 ...
 let pretty fmt wp =
    begin
      ...
      pp_gvcs fmt (gvcs_map wp.vcs);
    end

  let collect_bundle_ wp = gvcs_collect_ wp.vcs []

  let collect_bundle__ wp = gvcs_collect__ wp.vcs []       

  let collect_bundle wp = List.append (gvcs_collect_goal wp.vcs []) (extractBundle (gvcs_collect wp.vcs []) [])
      
(*List.append (gvcs_collect_goal wp.vcs []) *)


  let collect_bundle_hs_ wp =
      gvcs_collect wp.vcs []

  let collect_bundle_hs wp =
      extractBundle (gvcs_collect wp.vcs []) []

 let pp_bundle fmt wp = 
      let bundle_lst = collect_bundle_hs_ wp in
      List.iter (Format.fprintf fmt "@ @[<hov 2>Prove %a@]"
      Pcond.dump) bundle_lst

  let rec conjunction l = 
      p_conj l

  let pp_bundle_pred fmt  wp = 
      let bPred = conjunction (collect_bundle wp) in
      Format.fprintf fmt "bundle pred list = @ @[<hov 2> %a@]\n" F.pp_pred bPred
      
  let pp_bundle_pred_lst fmt wp = 
      let predList = collect_bundle wp in
      List.iter (Format.fprintf fmt "bundle pred list = @ @[<hov 2> %a@]\n" F.pp_pred) predList
      
  let convert_pred tpl =  let lst = List.map (fun obj -> p_conj (collect_bundle obj)) tpl in
                          let () = List.iter (debug "predicates to be converted: %a@" Lang.F.pp_pred) lst in
                          (*let pre1 = p_conj (W.collect_bundle !original_obj) in
                          let pre2 = p_conj (W.collect_bundle !inserted_obj) in*)
                          let pl = Lang.F.update_term_name lst in
                          List.iter (debug "convert predicates: %a@" Lang.F.pp_pred) pl (*reconstruction of formula here*)

  let rec printLst lst = match lst with
                     | pred :: lst_ -> debug2 "Proof obligation after the name unification: %a@" F.pp_pred pred; printLst lst_
                     | _ -> ()

  (*TODO: refine to distinguish multiple proof obligation*)
  let convert_pred init_obj inserted_obj = 
                                           (*let (pred_whens, goals, pred_nowhens) = collect_bundle_ init_obj in*)
                                           let pred_init = p_conj (collect_bundle_ init_obj) in
                                           let () = debug "init_obj: %a\n" pretty init_obj in
                                           let () = debug2 "Proof obligation of the original program: %a\n" Lang.F.pp_pred pred_init in
                                           (*let pred_hypo_insert = p_conj (collect_bundle_hs inserted_obj) in*)
                                           let pred_hypo_insert_lst = collect_bundle__ inserted_obj in
                                           let () = debug "insert_obj: %a\n" pretty inserted_obj in
                                           (*let () = debug "predicates to be converted hypo2: %a\n" Lang.F.pp_pred pred_hypo_insert in
                                           let pred_goal_insert = p_conj (gvcs_collect_goal inserted_obj.vcs []) in
                                           let () = debug "predicates to be converted goal: %a\n" Lang.F.pp_pred pred_goal_insert in*)
                                           (*let pred_inserted = p_imply pred_hypo_insert pred_goal_insert in*)
                                           let pred_inserted = p_conj pred_hypo_insert_lst in
                                           let () = debug2 "Proof obligation of the modified program: %a\n" Lang.F.pp_pred pred_inserted in
                                           let pl = Lang.F.update_term_name [pred_init;pred_inserted] in 
					   let () = printLst pl in 
                                           (*let () = List.iter (debug2 "convert predicates1: %a@" F.pp_pred) pl in reconstruction of formula here*)
                                           (*let () = Printf.printf "predicate list length:%d\n" (List.length pl) in*)
                                           let pi = imply_chain pl  in 
                                           debug2 "Proof obligation after integration: %a@" F.pp_pred pi (*reconstruction of formula here*)
  ...
  let assume_vc ?descr ?hpid ?stmt ?warn ?(filter=false) ?(init=false) hs vc =
    ...
    then 
      let () = ignore (debug "enter assume_vc_vc_999 = @[<hov2>  %a@]@." pp_vc vc) in
      vc 
      else
      let path = match stmt with
        | None -> vc.path
      ...
      let hyps = Conditions.assume
          ...
          (F.p_conj hs) vc.hyps
      in
      let () = ignore (debug "assume_hs_9995 = @[<hov2>  %a@]@." pp_pred (F.p_conj hs)) in
      let () = ignore (debug "assume_vc_hyps_9993 = @[<hov2>  %a@]@." Pcond.dump vc.hyps) in
      let () = ignore (debug "assume_new_hyps = @[<hov2>  %a@]@." Pcond.dump hyps)
      in
      let wp = 
      {
        hyps = hyps ;
      ...
      let () = ignore (debug "exit_assume_vc_prop_t = @[<hov2>  %a@]@." pp_vc wp) in
      wp

  let assume_vc_loop_cond ?descr ?hpid ?stmt ?warn ?(filter=false) ?(init=false) hs vc =
    if (hs = [] && warn = None) ||
       (filter && not (List.exists (intersect_vc vc) hs))
    then 
      let () = ignore (debug "enter assume_vc_vc_999 = @[<hov2>  %a@]@." pp_vc vc) in
      ...
      let wrns = match warn with
        | None -> vc.warn
        | Some w -> Warning.Set.union w vc.warn in
      let hyps = Conditions.assume_when
          ?descr ?stmt ?warn ~deps ~init
          (F.p_conj hs) vc.hyps
      in
      let () = ignore (debug "assume_hs_9995 = @[<hov2>  %a@]@." pp_pred (F.p_conj hs)) in
  ...
  (*mainly used for ITE structure*)
  let passify_vc pa vc =
    let () = ignore (debug "passify = @[<hov2>  %a@]@." pp_vc vc) in
    let hs = Passive.conditions pa (occurs_vc vc) in
    let () = ignore (debug "passify = @[<hov2>  %a@]@." pp_pred (F.p_conj hs)) in
    assume_vc hs vc
  ...
  let branch_vc ~stmt ~warn cond vc1 vc2 =
    (*let () = Printf.printf "branch_vc" in*)
    let hyps , goal =
      if F.eqp vc1.goal vc2.goal then
        begin
          (*let () = Printf.printf "branch_vc not loop" in*)
          Conditions.branch ~stmt ~warn cond vc1.hyps vc2.hyps ,
          vc1.goal
        end
      else
        (*let () = Printf.printf "branch_vc loop" in*)
        let k = F.e_var (Lang.freshvar ~basename:"K" Logic.Bool) in
        let p = F.p_equal k F.e_true in
        let q = F.p_equal k F.e_false in
        let () = ignore (debug "branch_vc p= @[<hov2>  %a@]@." pp_pred p) in
        let () = ignore (debug "branch_vc q= @[<hov2>  %a@]@." pp_pred q) in
  ...
  let add_vc target ?(warn=Warning.Set.empty) pred vcs =
    ...
    let res = Gmap.add target (Splitter.singleton vc) vcs in
    let () = (match Gmap.find target res with
              | vc -> ignore (debug "add_vc vc = @[<hov2>  %a@]@." pp_vcs vc)
             ) in
    res
  ...
  let add_hyp wenv (hpid,predicate) wp = in_wenv wenv wp
      (fun env wp ->
         ...
         let () = ignore (debug "add_hyp_999 = @[<hov2>  %a@]@." Cil_printer.pp_predicate predicate) in 
         let warn,hs = match outcome with
           | Warning.Result(warn,p) -> warn , [p]
           | Warning.Failed warn -> warn , []
         in
         let () = ignore (debug "assume_999_hyp_hs = @[<hov2>  %a@]@." pp_pred (F.p_conj hs)) in
          let () = ignore (debug "assume_999_hyp_vcs_be = @[<hov2>  %a@]@." pp_gvcs wp.vcs) in
         let vcs = gmap (assume_vc ~hpid ~warn hs) wp.vcs in
          let () = ignore (debug "assume_999_hyp_vcs_af = @[<hov2>  %a@]@." pp_gvcs vcs) in
          let wp = { wp with vcs = vcs } in
          let () = ignore (debug "add_hyp test printing bundles = @[<hov2>  %a@]@." pp_bundle wp ) in
        wp )
  ...
  let add_goal wenv (gpid,predicate) wp = in_wenv wenv wp
      (fun env wp ->
         ...
         let () = ignore (debug "add_goal_999 = @[<hov2>  %a@]@." Cil_printer.pp_predicate predicate) in 
         let warn,goal = match outcome with
           | Warning.Result(warn,goal) -> warn,goal
           | Warning.Failed warn -> warn,F.p_false
         in
         let () = ignore (debug "add_goal_9992 = @[<hov2>  %a@]@." pp_pred goal) in 
  ...
  let use_assigns wenv stmt hpid ainfo wp = in_wenv wenv wp
      begin fun env wp ->
        ...
        | WritesAny -> (*ignore (Printf.printf "use_assigns any\n");*)
        ...
        | Writes froms -> (*ignore (Printf.printf "use_assigns writes from\n");*)
            ...
            in
            match outcome with
            | Warning.Result(warn,(sequence,assigned,unfolded)) -> (*ignore (Printf.printf "use_assigns writes from results\n");*)
                let vcs =
            ...
            | Warning.Failed warn -> (*ignore (Printf.printf "use_assigns writes from warn\n");*)
                let sigma = Sigma.havoc_any ~call:false (L.current env) in
  ...
  let label wenv stmt label wp =
    let () = (debug "label called = @[<hov2>  %a@]@." Clabels.pretty label) in 
    if Clabels.is_here label then wp else
      in_wenv wenv wp
        (fun env wp ->
           ...
           let () = (debug "label passive = @[<hov2>  %a@]@." Passive.pretty pa) in 
           let () = (debug "label s_here = @[<hov2>  %a@]@." Sigma.pretty s_here) in 
           let () = (debug "label s_labl = @[<hov2>  %a@]@." Sigma.pretty s_labl) in 
  ...
  let cc_lval env lv =
    ...
    let () = (debug "cc_lval c_object obj = @[<hov2>  %a@]@." Ctypes.pp_object obj) in (*lv's data type*)
    (*pp_object*)
    (*lval:lhost * offset*)
    let dummy = Sigma.create () in
    let () = ignore (debug "cc_lval_sigma_dummy = @[<hov2>  %a@]@." Sigma.pretty dummy) in
    let l0 = C.lval dummy lv in
    (*env type:     (*LogicCompiler*)*)
    (*type env = {
   ...
   *)
    ...
    let () = ignore (debug "cc_lval_sigma_post = @[<hov2>  %a@]@." Sigma.pretty s2) in
    (*L:LogicSemantics, L.current = LogicCompiler.Make(M).current, M : Sigs.Model, getsigma env.current*)
    (*extract sigma from current env*)
    let domain = M.domain obj l0 in
    let s1 = Sigma.havoc s2 domain in
    (*let () = Printf.printf "cc_lval havoc called end" in*)
    let () = ignore (debug "cc_lval_sigma_pre = @[<hov2>  %a@]@." Sigma.pretty s1) in
    let loc = C.lval s1 lv in
    let seq = { pre=s1 ; post=s2 } in
    obj , domain , seq , loc

  (*return type: Sigs.equation list*)
  let cc_stored lv seq loc obj expr =
    ...
    then None
        ...
        begin
          ...
          | Lval lv -> let () = ignore (debug "lval") in 
                            M.copied seq obj loc (C.lval seq.pre lv)
          | _ ->      let () = ignore (debug "cc_stored loc = @[<hov2>  %a@]@." M.pretty loc) in (*pp_mem*) M.stored seq obj loc (C.val_of_exp seq.pre expr) (*c.val_o...: expr to term*)
        end


  let assign wenv stmt lv expr wp = in_wenv wenv wp
      begin fun env wp ->
        (debug "wp_assign_stmt = @[<hov2>  %a@]@." Printer.pp_stmt stmt);
        (debug "wp_assign_lval = @[<hov2>  %a@]@." Printer.pp_lval lv);
        (debug "wp_assign_exp = @[<hov2>  %a@]@." Printer.pp_exp expr);
        (debug "wp_assign_t_prop = @[<hov2>  %a@]@." pretty wp);
        let outcome = Warning.catch
        ...
        | Warning.Result(l_warn,(obj,dom,seq,loc)) ->
            ...
            (*obj: C_types.c_object*)
            (*dom: C.M.domain = C.M.Heap.set*)
            (*loc: C.C.loc = C.M.loc*)
            (*C : Sigs.Compiler *)
            (*M: Model(underlying memory model), C: CodeSemantics*)
            (*type loc (** Representation of the memory location in the model. *)*)
            (debug "wp_assign_after_cc_lval_t_prop = @[<hov2>  %a@]@." pretty wp);
            ...
            | Warning.Result(r_warn,None) ->
                ...
                let obj = 
                { sigma = Some seq.pre ; vcs=vcs ; effects = wp.effects } in
                (debug "wp_assign_no_stored = @[<hov2>  %a@]@." pretty obj);
                obj
            | Warning.Result(r_warn,Some stored) ->
                ...
                in
                let eqs1 = List.map Cvalues.equation stored in
                let update vc =
                  if List.exists (occurs_vc vc) ft
                  then 
                    let eqs = List.map Cvalues.equation stored in
                    let () = ignore (debug "wp_sigma_pre = @[<hov2>  %a@]@." Sigma.pretty seq.pre) in
                    let () = ignore (debug "wp_sigma_post = @[<hov2>  %a@]@." Sigma.pretty seq.post) in 
                    let () = ignore (debug "assume_eqs = @[<hov2>  %a@]@." pp_pred (F.p_conj eqs)) in               
                    assume_vc ~stmt ~warn eqs vc
                  else vc in
                let vcs = gmap update wp.vcs in
                let vcs =
                  check_assigns (Some stmt) FromCode region wp.effects vcs in
               (*let eqs = List.map Cvalues.equation stored in stored is a set of equations, such as x:=y or assertions*)
                    (*stored: [Set(Sigma.value seq.post chunk,mem)] equation:= Set(a,b) => p_equal a b*)
                    (*chunk represents the level value, need to use the index in seq.post, not seq.pre*)
                    (*  let value w c = e_var (get w c)*, evar := insert(Fvar x), Fvar x -> Var.hash x*)
                    (*eqs converts equations to predicates such as x == y *)                   
                let () = match eqs1 with
                                   | [eq] -> if ! Calculus.dag_flag then let (lh,offset) = lv in 
                                                                         match lh with
                                                                         |Var vi -> (*let () = ignore (Printf.printf "1120:%s\n" vi.vname)
                                                                                in *)
                                                                                    let () = ignore (debug "@[<hov2>  %a@]@" pp_pred eq) in
                                                                                    Lang.F.add_node eq  vi.vname (*add to dag node*)
                                                                         | _ -> Lang.F.add_node eq ""
                                   | _ -> ()
                             in
                let obj = 
                { sigma = Some seq.pre ; vcs=vcs ; effects = wp.effects } in
               (* (debug "assign test printing bundles = @[<hov2>  %a@]@." pp_bundle_pred obj) ;*)
                (debug "after_assign = @[<hov2>  %a@]@." pretty obj);
                obj
      end
  ...
  let return wenv stmt result wp =
    match result with
    | None ->  ignore (debug "return = @[<hov2>  %a@]@." pretty wp); wp
    | Some exp ->
        in_wenv wenv wp
          begin fun env wp ->
            ...
            let vr = L.result () in
            let () = match vr with
                    | R_var v -> debug "return vr = @[<hov2>  %a@]@." Lang.F.pp_var v
                    | _ -> debug "return vr = other"
            in 
            let tr = L.return () in
            let () = ignore (debug "return = @[<hov2>  %a@]@." Cil_types_debug.pp_exp exp) in
            let () = ignore (debug "return = @[<hov2>  %a@]@." Printer.pp_typ tr) in
  ...
  let rec obtainVar (p:Passive.t) = List.map (fun pa -> match pa with
                  | Passive.Bind(x,y) -> (x,y)
                  | Passive.Join(x,y) -> (x,y)) p
  ...
  let test wenv stmt exp wp1 wp2 = L.in_frame wenv.frame
      (fun () ->
         ...
         let () = ignore (match wp1.sigma with
                          | None -> ()
                          | Some sigma -> debug "test sigma union wp1 = @[<hov2>  %a@]@." Sigma.pretty sigma) in

         let () = ignore (match wp2.sigma with
                          | None -> ()
                          | Some sigma -> debug "test sigma union wp2 = @[<hov2>  %a@]@." Sigma.pretty sigma) in

         let () = ignore (debug "test sigma union = @[<hov2>  %a@]@." Sigma.pretty sigma) in
         let () = ignore (debug "test pa1 = @[<hov2>  %a@]@." Passive.pretty pa1) in
         let () = ignore (debug "test pa2 = @[<hov2>  %a@]@." Passive.pretty pa2) in
         (*pa1 and pa2*)
         (*no need to update namespace because pa1 and pa2 only contains vars that have been handled before*)
         let () = if ! Calculus.dag_flag then let () = debug "test pa1\n" in let xyl = obtainVar pa1 in List.iter (fun (x,y) -> add_node_eq x y) xyl in
         let () = if ! Calculus.dag_flag then let () = debug "test pa2\n" in let xyl = obtainVar pa2 in List.iter (fun (x,y) -> add_node_eq x y) xyl in
         let warn,cond =
         ...
         let vcs =
           if Wp_parameters.Split.get () then
             ...
             let vcs1 = gmap (passify_vc pa1) wp1.vcs in
             let () = ignore (debug "test vcs1 = @[<hov2>  %a@]@." pp_gvcs vcs1) in
             let vcs2 = gmap (passify_vc pa2) wp2.vcs in
             let () = ignore (debug "test vcs2 = @[<hov2>  %a@]@." pp_gvcs vcs2) in
             let () = ignore (debug "test cond = @[<hov2>  %a@]@." pp_pred cond) in
             gbranch
               ~left:(assume_vc_loop_cond ~descr:"Then" ~stmt ~warn [cond])
               ~right:(assume_vc_loop_cond ~descr:"Else" ~stmt ~warn [p_not cond])
  ...
  (*let call_add_node lv p= Lang.F.add_node p lv *)

  

  let call_proper wenv stmt lvr kf es ~pre ~post ~pexit ~assigns ~p_post ~p_exit () =
    let () = List.iter  (debug "call_proper pre= @[<hov2>  %a@]@." WpPropId.pp_pred_info) pre in
    let () = List.iter (debug "call_proper post= @[<hov2>  %a@]@." WpPropId.pp_pred_info) post in
    let call = cc_callenv wenv.main lvr kf es assigns p_post p_exit in (*callenv*)
    ...
    let kf_name = match kf.fundec with
                 | Definition (fd, loc) -> fd.svar.vname
                 | Declaration (_, fn, _, _) -> fn.vname in
    let () = debug "kf_name:%s\n" kf_name in
    (*add_node checks whether it is equality*)
    let () = if ! Calculus.dag_flag then List.iter (fun hs -> ignore (debug "call_proper add_node post = @[<hov2>  %a@]@." Lang.F.pp_pred hs); Lang.F.add_node hs kf_name) hs_post in

    let () = List.iter (debug "call_proper hs_pre = @[<hov2>  %a@]@." Lang.F.pp_pred) hs_pre in
    let () = List.iter (debug "call_proper hs_post = @[<hov2>  %a@]@." Lang.F.pp_pred) hs_post in
    let () = List.iter (debug "call_proper hs_exit = @[<hov2>  %a@]@." Lang.F.pp_pred) hs_exit in

    (* Binding result/status *)
    (*lv = result*)
    let hs_post_ = cc_result call in 
    let lv_name = match lvr with
                  | None -> ""
                  | Some (lh, offset) -> match lh with
                                        | Var vi -> vi.vname
                                        | _ -> ""
    in
    (*0430*)
    (*let () = if ! Calculus.dag_flag then List.iter (fun hs -> ignore (debug "call_proper fun = @[<hov2>  %a@]@." Lang.F.pp_pred hs); Lang.F.add_node hs lv_name) hs_post_ in*)
    let () = if ! Calculus.dag_flag then List.iter (fun hs -> ignore (debug "call_proper fun = @[<hov2>  %a@]@." Lang.F.pp_pred hs); 
    Lang.F.add_node hs lv_name;
    if hs_post == [] then
        Lang.F.add_name_fun kf_name
    else (*if no hs_post, not update dag*)
        () ) hs_post_ in
    let hs_post = hs_post_ @ hs_post in (*@ is list append*)
    let hs_exit = cc_status wenv.frame call.frame_exit :: hs_exit in
    let () = debug "call_proper hs_post length: %d" (List.length hs_post) in
    let () = List.iter (debug "call_proper hs_post2 = @[<hov2>  %a@]@." Lang.F.pp_pred) hs_post in
    let () = List.iter (debug "call_proper hs_pre2 = @[<hov2>  %a@]@." Lang.F.pp_pred) hs_pre in
    ...
    let () = ignore (debug "call_proper pa post pre sigma = @[<hov2>  %a@]@." Sigma.pretty call.sigma_pre) in

    let () = ignore (debug "call_proper pa post post sigma = @[<hov2>  %a@]@." Sigma.pretty call.seq_post.pre) in

    let pa_post = Sigma.join call.sigma_pre call.seq_post.pre in
    let pa_exit = Sigma.join call.sigma_pre call.seq_exit.pre in
 
    (*pa_post: list of Passive.binding*)

    (*type binding =
  | Bind of var * var (* fresh , bound *)
  | Join of var * var (* left, right *)*)

    let () =  if ! Calculus.dag_flag then let () = debug "test pa post\n" in let xyl = obtainVar pa_post in List.iter (fun (x,y) -> add_node_eq x y) xyl in

    (*var instance_k = var instance_m, if m < k, m depends on k*)

    (* Skip Precond for Caveat mode *)
    let hs_pre = if Wp_parameters.CalleePreCond.get () then hs_pre else [] in

    let () = ignore (debug "call_proper ceff vcs post = @[<hov2>  %a@]@." pp_gvcs ceff.vcs_post) in

    (*let () = debug "pa_post = @[<hov2>  %a@]@." Passive.pretty (List.hd pa_post) in*)
    ...
    let () = ignore (debug "call_proper cond_post = @[<hov2>  %a@]@." pp_gvcs cond_post) in
    ...
    let () = ignore (debug "call_proper vcs = @[<hov2>  %a@]@." pp_gvcs vcs) in
  ...
  let call wenv stmt lvr kf es ~pre ~post ~pexit ~assigns ~p_post ~p_exit
    = L.in_frame wenv.frame
      (fun () ->
         ...
         match outcome with
         | Warning.Result(warn , wp) -> let () = debug "W_call warn" in { wp with vcs = add_warnings warn wp.vcs }
         | Warning.Failed warn ->
             let () = debug "W_call failed" in
  ...
  let wp_scope env wp ~descr scope xs =
    ...
    let () = ignore (debug "wp_scope_assume_9996_post = @[<hov2>  %a@]@." Sigma.pretty post) in
    let pre = M.alloc post xs in
    let () = ignore (debug "wp_scope_assume_9996_pre = @[<hov2>  %a@]@." Sigma.pretty pre) in
    let hs = M.scope { pre ; post } scope xs in
    let () = ignore (debug "wp_scope_assume_9996_hs = @[<hov2>  %a@]@." pp_pred (F.p_conj hs)) in
    (*04/20/2020: disable scope vcs*)
    (*let vcs = gmap (assume_vc ~descr hs) wp.vcs in*)
    let vcs = wp.vcs in
    let () = ignore (debug "wp_scope_assume_99961_vcs = @[<hov2>  %a@]@." pp_gvcs vcs) in
  ...
  let make_vcqs target tags vc =
    ...
    let () = ignore (debug "make_vcqs = @[<hov2>  %a@]@." pp_vc vc) in
