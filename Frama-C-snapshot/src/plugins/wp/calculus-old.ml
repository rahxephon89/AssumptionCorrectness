...
module Cfg (W : Mcfg.S) = struct
  ...
  *)
  ...
  let findInsertion obj h =
    ...
    | _ -> ignore (Printf.printf "add hyp others")
    )) h
    ...
    let set strategy wenv res e obj =
      ...
      with Not_found ->
        begin
          ...
          let (id, p) = bh in
           match p.pred_content with 
    | Pfalse ->  ignore (Printf.printf "add hyp false")
    | Ptrue -> ignore (Printf.printf "add hyp true")
    | Papp (pi,labels,l) ->  ignore (Printf.printf "add hyp app: %s" pi.l_var_info.lv_name; 
                             if String.compare pi.l_var_info.lv_name "insertend" == 0  then 
                                       ignore(Lang.F.dag_copy (); Printf.printf "add hyp insertend: %s" pi.l_var_info.lv_name;  
                     debug "check obj insertend: @[<hov2>  %a@]@." W.pretty obj1;   original_obj := obj1)                               
                             else if String.compare pi.l_var_info.lv_name "insertbegin" == 0 then  
                                        ignore ( Printf.printf "add hyp insertbegin: %s" pi.l_var_info.lv_name; inserted_obj := obj1; compute_flag := true; dag_flag :=false;  debug "check obj insertbegin: @[<hov2>  %a@]@." W.pretty obj1;
                                       W.convert_pred  !original_obj !inserted_obj)
                                     else if String.compare pi.l_var_info.lv_name "dagbegin" == 0 then 
                                           ignore (Printf.printf "add hyp dagbegin true"; dag_flag := true) )
    | _ -> Printf.printf "add hyp others"
    ) bh_prop in           
  ...
  let wp_stmt wenv s obj = match s.skind with
    ...
    | Instr i ->
        begin match i with
          ...
          | (Call _) -> assert false
          | Skip _ | Code_annot _ -> Printf.printf "wp_stmt annotation"; obj
        end
